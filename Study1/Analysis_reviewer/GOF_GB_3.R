...
#GOF
wc_all$cutRt <- as.factor(cut(log(Rt_go_wb), quantile(log(Rt_go_wb),(0:3)/3), labels=1:3, include.lowest=T))
GOLD2 <- coxph(Surv(start,stop,CVD)~age + TC  + SEX  +  smoke + antihyp +sbp + bio + diabetess + cutRt, data=wc_all)

GOF<- try(wald.test(vcov(GOLD2),coef(GOLD2), Terms=9:10),silent=T)
...
#GOF
wc_s$cutRt <- as.factor(cut(log(Rt_wc_wb), quantile(log(Rt_wc_wb),(0:3)/3), labels=1:3, include.lowest=T))
tag <- 0
withCallingHandlers(
WC2 <- coxph(Surv(start,stop,CVD)~age + TC  + SEX  + smoke + antihyp +sbp + bio + diabetess + cutRt, data=wc_s),
warning=function(w) {tag <<-1})

if(tag==0) {GOF<- wald.test(u <- vcov(WC2),coef(WC2), Terms=9:10)
...
#GOF
cc_s$cutRt <- as.factor(cut(log(Rt_Pr_wb), quantile(log(Rt_Pr_wb),(0:3)/3), labels=1:3, include.lowest=T))
tag <- 0
withCallingHandlers(
PR2 <- coxph(Surv(start_cc,stop_cc,CVD_cc)~age+TC+ SEX  + smoke + antihyp + sbp + bio + diabetess +   cutRt + cluster(as.factor(keys)), data=cc_s),
warning=function(w) {tag <<-1})

if(tag==0) {GOF<- wald.test(vcov(PR2),coef(PR2), Terms=9:10)
...
#GOF
ncc$cutRt <- as.factor(cut(log(Rt_ncc_wb), quantile(log(Rt_ncc_wb),(0:3)/3), labels=1:3, include.lowest=T))
tag <- 0
withCallingHandlers(
NCC2 <- coxph(Surv(fakentry,rstime,cc)~age+TC+ SEX + smoke + antihyp + sbp + bio + diabetess + cutRt + offset(logw), data=ncc), 
warning=function(w) {tag <<-1})

if(tag==0) {GOF<- wald.test(vcov(NCC2),coef(NCC2), Terms=9:10)
...
#GOF
dstrat2$phase1$sample$variables$cutY <- as.factor(cut(log(Rt_BII_wb), quantile(log(Rt_BII_wb),(0:3)/3), labels=1:3, include.lowest=T))
...
if(tag==0) {GOF <- wald.test(vcov(BII_s42),coef(BII_s42), Terms=9:10)
...
#GOF
ncc_s$cutRt <- as.factor(cut(log(Rt_ncc_s_wb), quantile(log(Rt_ncc_s_wb),(0:3)/3), labels=1:3, include.lowest=T))
tag <- 0
withCallingHandlers(
NCC_s2 <- coxph(Surv(fakentry,rstime,cc)~TC + smoke + antihyp + sbp + bio + diabetess + cutRt + offset(logw), data=ncc_s), 
warning=function(w) {tag <<-1})

if(tag==0) {GOF <- wald.test(vcov(NCC_s2),coef(NCC_s2), Terms=7:8)
...
save <- list(F11_GOF)
save(save,file="GOF_bio1_1_3.Rdata")
...
save <- list(F13_GOF)
save(save,file="GOF_bio1_3_3.Rdata")
